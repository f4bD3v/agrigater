...
def reshaping(sheet_num, sheet):
	...
	sheet['Season'] = sheet['Season'].str.strip()
...
def prod_to_json(df):
	### do groupings
	prodjs = df.to_json(orient='records')
	return prodjs
...
def main():
	...
	print(df.tail())
	# TODO: clean seasons
	df.to_csv(open(path.join(data_dir, 'production_formatted.csv'), 'w'))

	prodjs = prod_to_json(df)
	json.dump(prodjs, open(path.join(data_dir, 'production.json', 'w')))
	# next: state names, district names and crops have to integrated
