...
def prepare_commodities(market_locations, mode, overwrite):
	...
	for folder in folders:
		...
		for filename in files:
			...
			df = df.rename(columns={'arrival': 'commodityTonnage'})
...
def prepare_states(locs):
	...
	selected_commodities = json.load(open(path.join(data_dir, 'commodities', 'selected_commodities.json'), 'r'))
	"""
...
def match_topo_dist_names(locs, state_match_dict, db):
	...
	district_json_dir = path.join(data_dir, 'admin', 'geo', 'districts_by_state')
	dfiles = os.listdir(district_json_dir)
	outdir = path.join(data_dir, 'integrated', 'admin', 'districts_by_state')
	if not path.isdir(outdir):
		os.makedirs(outdir)
	for filename in dfiles:
		if 'geojson' in filename or not 'json' in filename or 'Lakshadweep' in filename:
			continue
	#topojson = json.load(open(path.join(data_dir, 'admin', 'geo', 'districts_simp.json'), 'r'))
		from_here = path.join(data_dir, 'admin', 'geo', 'districts_by_state', filename)
		print(from_here)
		topojson = json.load(open(from_here, 'r'))
		print(topojson)
		### TODO: add an extra loop?
		#features = list(geojson['features'])
		#features = topojson['objects']['districts_simp']['geometries']
		features = topojson['objects'][filename.replace('.json', '')]['geometries']
		### GEOJSON DIFFERENTLY
		...
		for state in list(locs.keys()):
			...
			for shape in shapes:
				...
				### do not need id to retrieve district on geojson click => use state and district name
		...
		topojson['objects'][filename.replace('.json', '')]['geometries'] = mod_shapes
		with open(path.join(outdir, filename), 'w') as f:
			f.write(json_util.dumps(topojson))
			f.close()
...
def create_market_doc(row, logger, state_coords_df):
	...
	if isinstance(established, float) or int(established) == 0:
		...
		established = datetime.strptime(str(established), '%Y').date()
	...
	row.where((pd.notnull(row)), None)
	print(row)

	#coordinates, geojson = geocoding.get_market_coordinates(pin, row['Market_Cleaned'], row['District'], row['State'])
	coords_row = state_coords_df.loc[state_coords_df['Market'] == row['Market_Cleaned']]
	...
	coordinates = {
		'lat' : float(coords_row['Lat']),
		'lng' : float(coords_row['Lng'])
	}
...
def prepare_markets(cont = False):
	...
	print(os.getcwd())
	coordinates_fn = path.join(data_dir, 'admin', 'geo', 'market_coordinates.csv')
	coordinates_df = pd.DataFrame.from_csv(coordinates_fn, index_col=None)
	...
	for file in files:
		...
		print(file)
		df = pd.DataFrame.from_csv(file) # CAREFUL: index_col present
		print(df.head())
		...
		if os.path.isfile(outpath) and cont:
			continue
		state = list(df['State'].unique())[0]
		print(state)
		state_coords_df = coordinates_df.loc[coordinates_df['State'] == state]
		print(state_coords_df.head(10))
		print(outfile)
		print()
...
def prepare_market_features(db):
	outfile = path.join(data_dir, 'integrated', 'admin', 'markets.geojson')
